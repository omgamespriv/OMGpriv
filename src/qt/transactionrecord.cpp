...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return OmgGUI::tr("Received with");
    case RecvFromOther:
        return OmgGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return OmgGUI::tr("Sent to");
    case SendToSelf:
        return OmgGUI::tr("Payment to yourself");
    case Generated:
        return OmgGUI::tr("Mined");
    case RecvOmgX:
        return OmgGUI::tr("Received OMGX");
    case SendOmgX:
        return OmgGUI::tr("Sent OMGX");
    case Other:
        return OmgGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvOmgX:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendOmgX:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredOMG, nCredOmgX;
    wtx.GetCredit(nCredOMG, nCredOmgX, true);
    int64_t nCredit = nCredOMG + nCredOmgX;
    ...
    {
        if (nNet > 0 && nCredOmgX > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvOmgX, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendOmgX, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvOmgX;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv omgx
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendOmgX;
                    sub.address = CBitcoinAddress(ckidD).ToString();
