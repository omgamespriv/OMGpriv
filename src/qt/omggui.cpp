...
OmgGUI::OmgGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new OmgBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("OMG") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/omg1"));
    setWindowIcon(QIcon(":icons/omg1"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/omg/index.html");
#else
    QFile html("/opt/omg/index.html");
#endif
...
void OmgGUI::pageLoaded(bool ok)
{
...
void OmgGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);

}

void OmgGUI::urlClicked(const QUrl & link)
{
...
void OmgGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/omg"), tr("&About OMG"), this);
    aboutAction->setToolTip(tr("Show information about OMG"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for OMG"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/omg"), tr("&Show / Hide"), this);
...
void OmgGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("OMG") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/omg_testnet"));
            setWindowIcon(QIcon(":icons/omg_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/omg_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Omg client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/omg_testnet"));
...
void OmgGUI::setWalletModel(WalletModel *walletModel)
{
...
void OmgGUI::setMessageModel(MessageModel *messageModel)
{
...
void OmgGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("OMG client"));
    trayIcon->setIcon(QIcon(":/icons/omg"));
...
#ifndef Q_OS_MAC
void OmgGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void OmgGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to Omg network", "", count));
}

void OmgGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void OmgGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void OmgGUI::changeEvent(QEvent *e)
{
...
void OmgGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void OmgGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::OMG, nFeeRequired));
...
void OmgGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void OmgGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void OmgGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void OmgGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void OmgGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void OmgGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Omg address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void OmgGUI::handleURI(QString strURI)
{
...
void OmgGUI::setEncryptionStatus(int status)
{
    ...
    case WalletModel::Unlocked:
        ...
        if (fWalletUnlockStakingOnly || fWalletUnlockMessagingEnabled)
        {
            ...
            toggleLockIcon  .removeClass("omg-blue");
        ...
        } else
        {
            ...
            toggleLockIcon     .addClass("omg-blue");
        };
...
void OmgGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void OmgGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void OmgGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void OmgGUI::updateWeight()
{
    if (!pwalletMain)
        return;
