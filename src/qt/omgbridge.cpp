...
#include "omgbridge.moc"

OmgBridge::OmgBridge(OmgGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
// This is just a hook, we won't really be setting the model...
void OmgBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void OmgBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void OmgBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void OmgBridge::populateOptions()
{
...
// Transactions
bool OmgBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void OmgBridge::clearRecipients()
{
    recipients.clear();
}

bool OmgBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    {
        int inputType; // 0 OMG, 1 OMGX
        switch(rcp.txnTypeInd)
        {
            case TXT_OMG_TO_OMG:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::OMG, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_OMG_TO_ANON:
                formatted.append(tr("<b>%1</b> to OMGX %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::OMG, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> OMGX, ring size %2 to OMGX %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::OMG, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_OMG:
                formatted.append(tr("<b>%1</b> OMGX, ring size %2 to OMG %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::OMG, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::OMG, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsOmgXBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your OMGX balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::OMG, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void OmgBridge::openCoinControl()
{
...
void OmgBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void OmgBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap OmgBridge::listAnonOutputs()
{
...
void OmgBridge::populateTransactionTable()
{
...
void OmgBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void OmgBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString OmgBridge::transactionDetails(QString txid)
{
...
// Addresses
void OmgBridge::populateAddressTable()
{
...
void OmgBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void OmgBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString OmgBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString OmgBridge::lastAddressError()
{
    QString sError;
...
QString OmgBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void OmgBridge::updateAddressLabel(QString address, QString label)
{
...
bool OmgBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool OmgBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void OmgBridge::appendMessages(QVariantList messages, bool reset)
{
    emitMessages(messages, reset);
}

void OmgBridge::appendMessage(int row)
{
...
void OmgBridge::populateMessageTable()
{
...
void OmgBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool OmgBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool OmgBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString OmgBridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);
}

QString OmgBridge::addressForPubKey(QString pubkey)
{
    return addressModel->atm->addressForPubkey(pubkey);
}

bool OmgBridge::setPubKey(QString address, QString pubkey)
{
...
bool OmgBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
...
QString OmgBridge::createGroupChat(QString label)
{
...
QString OmgBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList OmgBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
QString OmgBridge::translateHtmlString(QString string)
{
    int i = 0;
...
QVariantMap OmgBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap OmgBridge::listLatestBlocks()
{
...
QVariantMap OmgBridge::findBlock(QString searchID)
{
...
QVariantMap OmgBridge::blockDetails(QString blkHash)
{
...
QVariantMap OmgBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap OmgBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap OmgBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap OmgBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap OmgBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap OmgBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap OmgBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap OmgBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap OmgBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with Omg prefix.");
            return result;
        }
...
QVariantMap OmgBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap OmgBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap OmgBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
