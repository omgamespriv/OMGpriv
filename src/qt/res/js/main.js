...
var overviewPage = {
    ...
    updateBalance: function(balance, omgXBal, stake, unconfirmed, immature) {
        if(balance == undefined)
            balance     = this.balance    .data("orig"),
            omgXBal   = this.omgXBal  .data("orig"),
            ...
            this.omgXBal  .data("orig", omgXBal),
        ...
        this.formatValue("omgXBal",   omgXBal);
    ...
    updateTransaction: function(txn) {
        var format = function(tx) {
            ...
            return "<a id='"+tx.id.substring(0,17)+"' data-title='"+tx.tt+"' class='transaction-overview' href='#' onclick='$(\"#navitems [href=#transactions]\").click();$(\"#"+tx.id+"\").click();'>\
                                                <span class='"+(tx.t == 'input' ? 'received' : (tx.t == 'output' ? 'sent' : (tx.t == 'inout' ? 'self' : 'stake')))+" icon no-padding blue-omg'>\
                                                 "+(tx.t == 'mined' ? "<img src=\"qrc:///icons/omg\" align=\"absmiddle\" class=\"iw-mIcon\" style=\"width:14px; height:14px; padding-right:8px; margin-top: 0px; margin-bottom:8px; margin-left:-3px\"/>" : "<i class=\"fa fa-"+(tx.t == 'input' ? 'angle-left' : (tx.t == 'output' ? 'angle-right' : 'angle-down'))+" font-26px margin-right-10\"></i>")
    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                ...
                <input id="pay_to[count]" class="pay_to input_box" data-title="The address to send the payment to  (e.g. MXywGBZBowrppUwwNUo1GCRDTibzJi7g2M)" placeholder="Enter a OMG address (e.g. MXywGBZBowrppUwwNUo1GCRDTibzJi7g2M)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
            ...
            <div class="flex-left"> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="Omg"                    ' + (unit.type == 0 ? "selected" : "") + '>OMG</option> \
                    <option value="1" title="Milli-Omg (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mOMG</option> \
                    <option value="2" title="Micro-Omg (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;OMG</option> \
                    <option value="3" title="Omglet (1 / 100000000)" ' + (unit.type == 3 ? "selected" : "") + '>Omglet</option> \
                </select> \
            </div> \
...
function changeTxnType()
{
    ...
    if (type > 1)
    {
        ...
        $("#coincontrol,#spend_omg").hide();
        $("#spend_omgX").show();
        $("#spend_omgX > .unit").html("OMGX")
        toggleCoinControl(false);
    }
    else
    {
        $("#tx_ringsize,#suggest_ring_size,#spend_omgX").hide();
        $("#coincontrol,#spend_omg").show();
        $("#spend_omg > .unit").html("OMG")
    }
