...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help"))
        {
            ...
            std::string strUsage = _("OMG version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  omgd [options]                     " + "\n" +
                  "  omgd [options] <command> [params]  " + _("Send command to -server or omgd") + "\n" +
                  "  omgd [options] help                " + _("List commands") + "\n" +
                  "  omgd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "omg:"))
                fCommandLine = true;
