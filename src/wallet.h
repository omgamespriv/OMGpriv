...
public:
    ...
    bool SendOmgToAnon(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    bool SendAnonToOmg(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    int64_t GetOmgXDebit(const CTxIn& txin) const;
    int64_t GetOmgXCredit(const CTxOut& txout) const;
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nOMG, int64_t& nOmgX) const
    {
        nOMG = 0;
        nOmgX = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nOmgX += GetOmgXCredit(txout);
            } else
                nOMG += GetCredit(txout);
            if (!MoneyRange(nOMG)
                || !MoneyRange(nOmgX))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
...
public:
    ...
    mutable bool fAvailableOmgXCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableOmgXCreditCached;
    
    mutable int64_t nCredOMGCached;
    mutable int64_t nCredOmgXCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableOmgXCreditCached = false;
    ...
    bool GetCredit(int64_t& nCredOMG, int64_t& nCredOmgX, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredOMG = nCredOmgX = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredOMGCached = 0;
            nCredOmgXCached = 0;
            pwallet->GetCredit(*this, nCredOMGCached, nCredOmgXCached);
            fCreditSplitCached = true;
        };
        
        nCredOMG = nCredOMGCached;
        nCredOmgX = nCredOmgXCached;
        return true;
    }
    ...
    int64_t GetAvailableOmgXCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableOmgXCreditCached)
            return nAvailableOmgXCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableOmgXCredit() : value out of range");
            };
        };
        
        nAvailableOmgXCreditCached = nCredit;
        fAvailableOmgXCreditCached = true;
        return nCredit;
    };
